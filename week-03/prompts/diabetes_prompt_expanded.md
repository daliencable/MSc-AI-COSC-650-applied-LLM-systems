...
Severity levels:
- P1 (Critical): failures causing incorrect insulin delivery, complete loss of critical alerts, corruption of automated dosing logic, or a malfunction that causes the patient to change their behavior in a way that undermines intended safety use (for example, disabling a safety alert)
- P2 (High): lost connectivity or data gaps that leave a patient without real-time tracking, forcing blind-management or manual workarounds, without a behavior change that removes a safety feature
...
When scoring severity, consider the worst possible consequence to the patient that the ticket implies, not just the immediate problem the user described. An issue that leads the patient to missing an alert or a treatment dose can lead to hospitalization or even death.
...
{"category": "device", "category_rationale": "This is a device issue as it's directly related to either a software, firmware, or hardware malfunction.", "severity": "P2", "severity_rationale": "Repeated disconnections create a real-time tracking gap, but the ticket doesn't describe any change in the patient's safety behavior."}
...
Ticket: "I took my short acting dose before eating but it's not showing up in the app log. Is there a way to manually enter it?"
{"category": "device", "category_rationale": "The pen cap failed to automatically register a delivered dose, even though the patient used the system. This isn't a treatment failure but a malfunction with either the pen cap firmware not recognizing being removed and returned to the pen, a firmware or hardware communication failure between the pen and the app, or an app software error in logging it.", "severity": "P1", "severity_rationale": "Anyone relying on the log to know whether they've dosed could double-dose if the log silently drops an entry, not just patients with memory issues, and this could lead to hospitalization or death."}
